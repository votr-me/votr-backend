...
class ACS5CRUD:
    ...
    async def get_congerssional_district_asc5_demographics(
        self, db: AsyncSession, district_num: int, state_district_num: str
    ):
        
        query = select(ACS5Demographics)\
                    .filter(ACS5Demographics.state_fip == state_district_num)\
    ...
    #     query = select(ACS5Employment)\
    #                 .filter(ACS5Employment.state_fip == state_district_num)\
