...
class CongressAPIAsyncClient(BaseAsyncAPIClient):
    ...
    async def get_member_info(self, bioguideId: str) -> Dict[str, Any]:
        """
    ...
    async def get_sponsored_legislation(
        self, bioguideId: str, limit: int, offset: int
    ) -> Dict[str, Any]:
        """
    ...
    async def get_all_sponsored_legislation(
        ...
        logger.debug(f"Retrieving first {offset}, will retrieve {total_count}")

        while len(all_legislation) < total_count:
            try:
                logger.debug(
                    f"Retrieving bills from: {response.get('pagination', {}).get('next', None)}"
                )
                response = await self.get(
                    endpoint=response.get("pagination", {}).get("next", None),
                )
