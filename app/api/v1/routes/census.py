...
from app.schemas.census.acs5 import ACS5DemographicsSchema
...
async def read_acs5_data(
    ...
    if not state_fips in VALID_STATE_FIPS_CODES:
        ...
        if not district_num in VALID_CONGRESSIONAL_DISTRICTS_BY_STATE_FIPS.get(state_fips, {}):
            logger.debug(f"District {district_num} does not exist for State FIPS code {state_fips}")
            
        else:
            logger.debug(f"Fetching data for congressionl district {district_num} in {state_fips}")

    redis_key = f'{state_fips}_{district_num}'
    try:
        ...
        state_district_demos_db = await ACS5CRUD()\
                                    .get_congerssional_district_asc5_demographics(
                                        db=db, 
                                        district_num=district_num, 
                                        state_district_num=state_fips
                                        )
        
        if not state_district_demos_db :
            logger.info(f"No data found for {state_fips} district {district_num}")
            raise HTTPException(status_code=404, detail=f"No data found for {state_fips} district {district_num}")

        # Convert to Pydantic model
        result = [ACS5DemographicsSchema.from_orm(record).model_dump(by_alias=True) for record in state_district_demos_db]
